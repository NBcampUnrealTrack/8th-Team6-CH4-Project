...
namespace UEBridgeMCP_PieInputInternal
{
	...
	static void InjectKeyEvent(APlayerController* PC, const FKey& Key, EInputEvent Event, float AmountDepressed, FViewport* Viewport)
	{
		...
		const FInputKeyParams Params(Key, Event, static_cast<double>(AmountDepressed), false, DeviceId);
		...
		{
			const FInputKeyParams AxisParams(Key, IE_Axis, static_cast<double>(AmountDepressed), false, DeviceId);
			PC->InputKey(AxisParams);
		}
	}
